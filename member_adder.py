...
from telethon.errors.rpcerrorlist import PeerFloodError, UserPrivacyRestrictedError, PhoneNumberBannedError, FloodWaitError
...
success, message = ensure_directory_exists(members_dir, 'members')
if not success:
    print(f'{error}{message}')
    sys.exit()
...
for i in accounts[:a]:
    ...
    to_use.append(i)
    file = os.path.join(BASE_DIR, 'members', 'members' + str(accounts.index(i)) + '.csv')
...
for account in to_use:
    ...
    api_hash = str(account[1])
    phone = str(account[2])
    file = os.path.join(BASE_DIR, 'members', 'members' + str(to_use.index(account)) + '.csv')
    os.system('start cmd')
